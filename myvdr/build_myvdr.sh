...
make_customize_root_image() {
    if [[ ! -e ${work_dir}/build.${FUNCNAME} ]]; then
        ...
        chroot ${work_dir}/root-image /usr/sbin/useradd -m -p "" -g users -G "video,audio,disk,optical,wheel,storage,uucp" myvdr
        chroot ${work_dir}/root-image /bin/chown -R myvdr.users /home/myvdr

        chroot ${work_dir}/root-image /bin/bash -c "echo 'root:root' | chpasswd"
        chroot ${work_dir}/root-image /bin/bash -c "echo 'myvdr:myvdr' | chpasswd"
        chmod 600 ${work_dir}/root-image/etc/shadow

        chmod 750 ${work_dir}/root-image/etc/sudoers.d
        chmod 440 ${work_dir}/root-image/etc/sudoers.d/myvdr
